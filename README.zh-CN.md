...
**运维**
- **MCP 服务器** 提供搜索、摘要、digest、决策、会话、issue 查找和文件访问等工具
- **回发消息**（按源选择性开启）—— 支持 Telegram business 聊天；邮件回复落到你的 Drafts 文件夹供审阅
- **保留 / housekeeping** —— 自动清理旧消息和向量；内容寻址的附件清扫机制保留所有仍被引用的文件
- **CLI**: `./bin/memorandum {health, dashboard, aliases refresh, prune}` —— 实时终端 TUI 加运维工具
...
编辑 `config.yaml`（步骤 2 中从 `config.example.yaml` 生成），添加你的源：
...
sources:
  ...
  work_telegram:
    type: telegram
    enabled: true
    token: "123456:AABBcc..."   # 来自 @BotFather

  work_pachca:
    type: pachca
    enabled: true
    token: "your-personal-access-token"   # Pachca 设置中的 Automations → API
    filters:
...
测试套件（约 310 个用例）覆盖了配置加载、过滤、SQLite 存储、MCP URL 生成与工具处理器、三种连接器（HTTP 通过 `responses` mock）、ingest 编排器（VectorStore 被 mock —— 不加载 BGE-M3 模型）、CLI 调度器，以及 `aliases refresh` 通过 `ruamel.yaml` 的 round-trip。
...
2. **从零开始。** 停掉 ingest，`rm -rf data/chroma/`，再 `./run_ingest.sh --hours <N>` 重新 embed 一切。
